...
class PeriodicCashReportController extends Controller
{
    public function profession()
    {        
        $client = Client::with('professions')->findOrFail(client()->id);
        if ($client->gst_method == 1) {
            return view('frontend.report.periodic_cash.profession', compact('client'));
        } else {
    ...
    {        
        ...
        return view('frontend.report.periodic_cash.date', compact(['profession', 'periods']));
    }
    public function report(Request $request, Client $client, Profession $profession, GstPeriodic $periodic)
    {
        ...
        if ($request->peroid_id) {
            ...
            return view('frontend.report.periodic_cash.report', $data);
        } else {
