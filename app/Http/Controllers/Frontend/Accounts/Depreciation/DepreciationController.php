...
namespace App\Http\Controllers\Frontend\Accounts\Depreciation;
...
class DepreciationController extends Controller
{
    public function index()
    {
        ...
        return view('frontend.accounts.depreciation.profession', compact('client'));
    }
    ...
    public function assetGroupIndex(Client $client, Profession $profession)
    {
        ...
        return view('frontend.accounts.depreciation.input', compact('client', 'profession', 'dep_accs', 'accm_dep_accs'));
    }
    ...
    public function assetGroupRead(Request $request)
    {
        ...
        if ($inputs->count() > 0) {
            foreach ($inputs as $input) {
                ...
                $html .= '<a data-id="' . $input->id . '" class="red btn btn-info btn-sm" id="update" href="' . route('client.dep_name.index', $input->id) . '">Add/Edit Asset</a>';
                $html .= '</td>';
                $html .= '</tr>';
            }
    ...
    public function assetNameIndex(Depreciation $dep)
    {
        $periods = Period::where('client_id', $dep->client_id)
            ...
            ->groupBy('year')
            ->get();
        return view('frontend.accounts.depreciation.add_asset', compact('dep', 'periods'/* , 'dep_assets' */));
    }
    ...
    public function assetNameRead(Request $request)
    {
        ...
        if ($inputs->count() > 0) {
            ...
            $asset_name = view('frontend.accounts.depreciation.ajax', ['inputs' => $inputs, 'type' => 'asset_name'])->render();

            $active_asset = view('frontend.accounts.depreciation.ajax', ['active_assets' => $active_assets, 'type' => 'active_asset_name'])->render();

            $old_asset = view('frontend.accounts.depreciation.ajax', ['old_assets' => $old_assets, 'type' => 'old_asset_name'])->render();

            return response()->json(['status' => 'success', 'html' => $asset_name, 'dep_asset_name' => $active_asset, 'old_asset_name' => $old_asset]);
        } else {
            $asset_name = view('frontend.accounts.depreciation.ajax', ['type' => 'error'])->render();
    ...
    public function activeAssetUpdate(Request $request)
    {
        ...
        if ($assetName->year == null) {
            ...
            $dep_amt = number_format($request->actual_depreciation / $yearDay, 4);
