                                ...
                                <div class="table table-responsive">
                                    <form action="{{route('bank_recon.store', $client->id)}}" method="post">
                                        ...
                                        <div class="pull-right py-5" style="padding: 15px 0">
                                            @php
                                            $is_disabled = $bank_recons->count() <= 0 ? 'disabled' : '' ; @endphp <input
                                                {{$is_disabled}} type="submit" name="type" value="Print/PDF" class="btn"
                                                ...
                                                <input disabled type="submit" name="type" value="Save" class="btn"
                                        ...
                                        <table class="table table-bordered table-striped">
                                            ...
                                            <tbody>
                                                ...
                                                $identifier = $recon['tran_id'] . $recon['code'] .
                                                abs_number($diff,2,'_').$date->format('Ymd');
                                                @endphp
                                                ...
                                                <input type="hidden" name="tran_id[]" value="{{$recon['tran_id']}}">
                                                <input type="hidden" name="code_name[]" value="{{$recon['name']}}">
                                                <input type="hidden" name="chart_id[]" value="{{$recon['code']}}">
                                                <input type="hidden" name="narration[]" value="{{$recon['narration']}}">
                                                <input type="hidden" name="debit[]" value="{{$recon['debit']}}">
                                                <input type="hidden" name="credit[]" value="{{$recon['credit']}}">
                                                <input type="hidden" name="identifier[]" value="{{$identifier}}">
                                                <input type="hidden" name="diff_array[]"
                                                    value="{{in_array($identifier,
                                                    $bank_recons->pluck('identifier')->toArray()) ? $bank_recons->where('identifier',$identifier)->first()->diff : 0}}"
                                                    id="diff_{{$identifier}}">
                                                <tr>
                                                    ...
                                                    <td>{{$credit}}</td>
                                                    <td>
                                                        <input type="checkbox" name="diff[]" data-id="{{$i}}" data-identifier="{{$identifier}}" id="diff_{{$i}}"
                                                            value="{{$debit != 0 ?(-$debit):$credit}}"
                                                            {{in_array($identifier,
                                                            $bank_recons->pluck('identifier')->toArray()) ? 'checked' :''}}>
                                                        &nbsp;
                                                        <label for="diff_{{$i}}">
                                                            {{$diff}}
                                                        </label>
                                            ...
                                            <tfoot>
                                                <tr>
                                                    <td colspan="100%">
                                                        <div class="pull-right py-5" style="padding: 15px 0">
                                                            @php
                                                            $is_disabled = $bank_recons->count() <= 0 ? 'disabled' : ''
                                                                ; @endphp <input {{$is_disabled}} type="submit"
                                                                ...
                                                                @endif
                                                                <input {{$is_disabled}} type="submit" name="type"
                                                                    value="Post" class="btn btn-warning">
                                                                @endif
                                                                @if ($posted_recons->count() > 0)
