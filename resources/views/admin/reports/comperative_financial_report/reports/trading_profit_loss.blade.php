        ...
        <div class="col-md-8 col-lg-offset-2" style="padding-top:40px; ">
            <table class="table table-bordered table-hover table-striped" style="margin: 0 auto; ">
                ...
                <tbody>
                    ...
                    @if($accountCode->additional_category_id == $additionalCategory->id && $data["tpl_ledgers"]->where('chart_id', $accountCode->code)->sum('balance') != 0 && $code != $accountCode->code)
                    @php
                    ...
                    if($subCategory->id == 16 && $accountCode->code == 299998){
                        $preSubSubGrpBalance += $data["prePlRetain"];
                        $subSubGrpBalance += $data["plRetain"];
                    }elseif($subCategory->id == 16 && $accountCode->code == 299999){
                        $preSubSubGrpBalance += $data["preRetain"] ;
                        $subSubGrpBalance += $data["retain"] ;
                    }
                    @endphp
                    <tr>
                        ...
                        {{$data["preRetain"]? number_format(($data["preRetain"]),2) : '0.00'}}
                        @elseif(optional($preLedger)->chart_id ==999998)
                        {{$data["prePlRetain"]? number_format(($data["prePlRetain"]),2) : '0.00'}}
                        @else
                        ...
                        {{$data["retain"]? number_format(($data["retain"]),2) : '0.00'}}
                        @elseif($ledger->chart_id ==999998)
                        {{$data["plRetain"]? number_format(($data["plRetain"]),2) : '0.00'}}
                        @else
                    ...
                    <tr style="text-align:right;color: #d35400;">
                        ...
                        <td>
                            <span style="border-top:1px solid;border-bottom-style:double;font-weight: bold;float: right;">
                    ...
                    <tr style="text-align:right; color: #262626">
                        ...
                        @php($pre_gross_profit = abs($preIncome) - abs($pre_trading)- abs($pre_manufacturing))
                        <td class="{{(($pre_gross_profit < 0)?'text-danger':'')}}">
                            {{ number_format(($pre_gross_profit),2) }}</td>

                        @php($gross_profit = abs($income) - abs($trading)- abs($manufacturing))
