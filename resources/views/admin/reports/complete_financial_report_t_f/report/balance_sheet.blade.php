                        ...
                        @foreach ($data['bs_accountCodeCategories'] as $accountCodeCategory)
                            ...
                            @foreach ($accountCodeCategory->subCategoryWithoutAdditional->sortBy('code') as $subCategory)
                                ...
                                @foreach ($subCategory->additionalCategory->sortBy('code') as $additionalCategory)
                                    ...
                                    {{-- Account Code End --}}
                                @endforeach
                                <tr style="{{ $subCategory->name == 'Capital & Equity' ? 'color:#d35400' : 'color: green' }}"
                                    class="text-right">
                                    <td>Total {{ $subCategory->name }}</td>
                                ...
                                @if ($addNewRow == 4)
                                    ...
                                    <tr style="color: #d35400">
                                        ...
                                        <td class="text-right">
                                            <span
                                                style="border-top:1px solid;border-bottom-style:double;font-weight: bold;">{{ nF2($data['total_asset'] - $data['total_liability']) }}</span>
                                        </td>
                                    </tr>
                                @endif
                            @endforeach
                            {{-- Sub Category End --}}
                            <tr
                                style="text-align:right;color: green; {{ $accountCodeCategory->name == 'Liability and Equity' ? 'display:none' : '' }}">
                                <td class="text-right" style="font-size: 15px;">Total {{ $accountCodeCategory->name }}
                                </td>
                                <td>
                                    <span
                                        style="border-top:1px solid;border-bottom:1px solid;font-weight: bold;float:right">
                                        @if ($accountCodeCategory->code == 5)
