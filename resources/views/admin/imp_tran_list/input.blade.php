                        ...
                        <div class="card">
                            <div class="card-body">
                                @if (!request()->only_view)
                                    <form
                                        action="{{ route('bs_tran_list.input.update', [$client->id, $profession->id, $tran_id]) }}"
                                ...
                                <table class="tbl">
                                    ...
                                    @foreach ($inputs as $input)
                                        <tr>
                                            <td>
                                                @php
                                                    $selectedCode = $input->client_account_code;
                                                @endphp
                                                <select class="form-control account_code" name="account_code[]"
                                                    data-route="{{ route('bs_tran_list.updateCode', $input->id) }}">
                                                    @foreach ($codes as $code)
                                                        <option value="{{ $code->id }}" style="color:{{ $code->type == 1 ? '#f542e9' : 'blue' }}"
                                            ...
                                            <td>
                                                ...
                                                <input name="debit[]" step="any" class="form-control" type="number"
                                                    min="0" step="any"
                                            ...
                                            <td>
                                                ...
                                                <input name="credit[]" class="form-control" type="number" min="0"
                                                    step="any" value="{{ number_format($input->credit, 2, '.', '') }}"
                                        ...
                                        <tr>
                                            <td colspan="7">
                                                <input onclick="return confirm('Are you sure?')" type="submit"
                                                    value="Update" class="form-control btn btn-primary"
