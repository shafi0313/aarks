                                        ...
                                        <div class="panel-body">
                                            <div class="col-md-12">
                                                <div class="col-md-4">
                                                    ...
                                                    <span style="color: red">If you cannot see any a/c code please update profession from the profile menu.</span>
                                                </div>
                                                    ...
                                                    <div class="form-group">
                                                        <select class="form-control chosen-select" id="subid" name="subid"
                                                            ...
                                                            @foreach ($account_codes as $ac_code)
                                                                @if ($ac_code->type ==1)
                                                                <option value="{{route('sub_pro_show',[$ac_code->id,$ac_code->code,$period->id,$profession->id,$client->id])}}"
                                                                    style="color: green;">
                                                                    {{$ac_code->name}}
                                                                </option>
                                                                @else
...
<tr>
    ...
    <td width="7.9%" height="36">
        @if($item->total_inv != 0 && $item->gst_accrued_debit == 0 && $item->gst_accrued_credit == 0 && $item->gst_cach_credit == 0)
        {{ $item->gst_cash_debit }}
        @endif
        @if($item->total_inv != 0 && $item->gst_accrued_debit == 0 && $item->gst_accrued_credit == 0 && $item->gst_cash_debit == 0)
        {{ $item->gst_cash_credit }}
        @endif
        @if($item->total_inv != 0 && $item->gst_accrued_debit == 0 && $item->gst_cash_credit == 0 && $item->gst_cash_debit
        == 0)
        {{$item->gst_accrued_credit}}
        @endif
        @if($item->total_inv != 0 && $item->gst_accrued_credit == 0 && $item->gst_cash_credit == 0 && $item->gst_cash_debit
        == 0)
        {{ $item->gst_accrued_debit }}
        @endif
    </td>
    <td width="8.8%">{{$item->total_inv== 0?0:$item->total_inv}}
    </td>
    <td width="9.8%">
    {{$item->balance== 0?0:abs($item->balance)}}</td>
    <td width="9.9%">{{$item->percent== 0?0:$item->percent}}
    </td>
    <td width="29.6%">{{$item->narration}}</td>
    <td width="5%">
        <form action="{{route('dataStore.destroy',$item->id)}}"
            method="POST">
            @csrf
            @method("DELETE")
            <button  title="Data Store Delete"
            style="background: transparent;border: none;color: red;font-size: 14px;line-height: 0;"
            type="submit" class=""><i class="fa fa-trash-o delete"
...
<tr>
    ...
    <td width="7.9%" height="36">
        @if($item->total_inv != 0 && $item->gst_accrued_debit == 0 && $item->gst_accrued_credit == 0 && $item->gst_cash_credit != 0)
        {{ $item->gst_cash_credit }}
        @endif
        @if($item->total_inv != 0 && $item->gst_accrued_debit == 0 && $item->gst_accrued_credit == 0 && $item->gst_cash_debit != 0)
        {{ $item->gst_cash_debit }}
