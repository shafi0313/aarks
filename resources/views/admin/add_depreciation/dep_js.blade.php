<script>
    ...
    $('#addAssetName').on('submit', (e)=>{
        e.preventDefault();
        $.ajax({
            url: '{{route("depreciation.name.store")}}',
            type: "post",
            data: {
                _token       : token,
    ...
    function readAssetName(){
        ...
        $.ajax({
            url:'{{route("depreciation.name.read")}}',
            method:'get',
            data:{
                dep_id:"{{$dep->id}}"
            },
    ...
    function getRecord(actionName, id) {
        $.ajax({
            url: '{{route("depreciation.name.edit")}}',
            method: 'get',
            data: { id: id },
    ...
    $('#update-modal').on('submit', (e)=>{
        e.preventDefault();
        $.ajax({
            url: '{{route("depreciation.name.update")}}',
            type: "post",
            data: {
                _token       : token,
    ...
    $("#asset_entryvalue").submit(function(e)
    {
        ...
        $.ajax({
            ...
            error:err=>{
                console.log(err);
                if(err.status == 422){
                    toast('error', 'Some Input Fields are not completed.');
                } else {
    ...
    function getDisposal(){
        $.get('{{route('depreciation.disposal.getAsset')}}',{
            client_id    : {{$dep->client_id}},
            profession_id: {{$dep->profession_id}},
            dep_id       : {{$dep->id}},
        }, res=>{
    ...
    function getDisposalModal(id){
        $.get('{{route('depreciation.disposal.getModal')}}',{
            id:id
        }, res=>{
