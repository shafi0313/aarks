...
<tr id="input_row_{{$input->id}}">
    ...
    <td style="text-align:center">
        <button data-id="{{$input->id}}" onclick="getRecord('update', {{$input->id}})" title="Edit Depriciation" class="_edit red btn btn-info btn-sm"
            data-route="{{route('depreciation.name.update', $input->id)}}">
            <i class="fa fa-pencil-square-o fa-lg" aria-hidden="true"></i>
        </button>
        @if ($input->rollover_year == null && $input->disposal_date == null)
        <button title="Delete Depriciation"  class="red btn btn-danger btn-sm ml-3 _delete" data-route="{{route('depreciation.name.destroy', $input->id)}}">
