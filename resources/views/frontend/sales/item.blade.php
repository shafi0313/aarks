<div class="row mx-auto">
    <div class="form-group mx-1" style="width: 190px">
        ...
        <select name="item_id" id="item_id" class="form-control">
            ...
            <option value="{{$item->id}}" account_id="{{$item->code->code}}" account_name="{{$item->code->name}}"
                data-item_type="{{$item->type}}" data-sell-price='{{$item->price}}' gst_id='{{$item->gst_code}}'
    ...
    <div class="form-group mx-1" style="width: 100px">
        ...
        <input type="number" name="quantity" step="any" id="quantity" oninput="this.value = this.value.replace(/[^\d]/g,'');"
            class="form-control">
    </div>
    <div class="form-group mx-1" style="width: 100px">
        <label style="font-size: 15px">Rate(Ex GST)</label>
        <input type="number" name="rate" step="any" id="rate" oninput="this.value = this.value.replace(/[^\d]/g,'');"
            class="form-control">
    </div>
    <div class="form-group mx-1" style="width: 100px">
        <label>Amount</label>
        <input readonly type="number" step="any" name="amount" id="amount" class="form-control">
    </div>
    <div class="form-group mx-1" style="width: 100px">
        <label>Disc %: </label>
        <input class="form-control" step="any" type="Number" name="disc_rate" id="disc_rate" placeholder="Disc %"
            oninput="this.value = this.value.replace(/[^\d]/g,'');">
    </div>
    <div class="form-group mx-1" style="width: 100px">
        <label style="font-size: 14px">Freight Charge: </label>
        <input class="form-control" step="any" type="Number" name="freight_charge" id="freight_charge"
            oninput="this.value = this.value.replace(/[^\d]/g,'');">
    </div>
    <div class="form-group mx-1" style="width: 150px">
        <label>Account Code: </label>
        <input type="hidden" name="chart_id" id="chart_id">
        <input type="hidden" name="item_reg_name" id="item_reg_name">
