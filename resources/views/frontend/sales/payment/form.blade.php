    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-9" style="border:1px solid #CCCCCC; min-height:500px; padding: 5px 30px">
                ...
                <form action="{{route('payment.store')}}" method="post" autocomplete="off" id="payStore">
                    @csrf
                    <input type="hidden" name="client_id" value="{{$client->id}}">
                    <input type="hidden" name="profession_id" value="{{$profession->id}}">
                    <input type="hidden" name="customer_id" value="{{$customer->id}}">
                    <input type="hidden" name="new_inv" value="{{$dedotrs->max('inv_no')+1}}">
                    <div class="row">
                        <div class="col-md-6">
                    ...
                    <div class="row">
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody>
                                ...
                                @foreach ($dedotrs->groupBy('inv_no') as $dedotr)
                                @php
                                    $dueAmt = $dedotr->sum('amount') - $dedotr->first()->payments->sum('payment_amount');
                                @endphp
                                @if($dueAmt > 0)
                                <tr>
                                    <td class="serial">{{$invsl+=1}}</td>
                                    <td align="center">
                                        <a href="#">
                                            {{invoice($dedotr->first()->inv_no, 8, 'INV')}}
                                            <input type="hidden" name="inv_no[]" value="{{$dedotr->first()->inv_no}}">
                                        </a>
                                    </td>
                                    <td align="center">{{$dedotr->first()->tran_date->format('d/m/Y')}}</td>
                                    <td align="center">
...
<!-- Footer End -->
<script>
    $("#bl_pay_amount").on('keyup',function(e){
        $("#openbl").val($(this).val())
    });
