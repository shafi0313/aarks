<div class="row mx-auto">
    <div class="form-group mx-1" style="width: 190px">
        ...
        <select name="item_id" id="item_id" class="form-control">
            ...
            @foreach ($category->items->where('type', 1) as $item)
            <option value="{{$item->id}}" account_id="{{$item->code->code}}" account_name="{{$item->code->name}} " data-item_type="{{$item->type}}" data-sell-price='{{$item->price}}' gst_id='{{$item->gst_code}}' data-item_reg_name ="{{\Str::slug($item->item_name)}}-{{$item->item_number}}" >{{$category->name}} => {{$item->item_name}}</option>
            @endforeach
            @endforeach
    ...
    <div class="form-group mx-1" style="width: 100px">
        <label>Disc %: </label>
        <input class="form-control" type="Number" name="disc_rate" id="disc_rate" placeholder="Disc %">
    </div>
    <div class="form-group mx-1" style="width: 100px">
        <label style="font-size: 14px">Freight Charge: </label>
        <input class="form-control" type="Number" name="freight_charge" id="freight_charge">
    </div>
