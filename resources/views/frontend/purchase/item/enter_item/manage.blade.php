                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-3">
                                    <strong style="color:green; font-size:20px;">Supplier Manage: </strong>
                                </div>
                                <div class="col-md-9">
                                    ...
                                    @foreach ($services->groupBy('inv_no') as $service)
                                        <tr>
                                            ...
                                            <td class="text-center text-primary">
                                                <a
                                                    href="{{ route('bill.report', ['item', $service->first()->inv_no, $client->id]) }}">{{ invoice($service->first()->inv_no, 8, 'BILL') }}</a>
                                            </td>
                                            ...
                                            <td class="text-success">$
                                                {{ number_format($service->first()->payments->sum('payment_amount'), 2) }}
                                            </td>
                                            <td>
                                                <div class="action">
                                                    <a title="Bill Print"
                                                        href="{{ route('bill.report', ['item', $service->first()->inv_no, $client->id]) }}"
                                                        class="btn btn-primary btn-sm">
                                                        <i class="fa fa-print"></i>
                                                    </a>

                                                    <a title="BILL Mail"
                                                        href="{{ route('bill.viewable_mail', ['item', $service->first()->inv_no, $client->id]) }}"
                                                        class="btn btn-primary btn-sm">
                                                        <i class="far fa-envelope-open"></i>
                                                    </a>
                                                    <a title="Item Edit"
                                                        href="{{ route('enter_item.billedit', [$service->first()->inv_no, $service->first()->client_id]) }}"
                                                        class="btn btn-info btn-sm">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <form action="{{ route('enter_item.destroy', $service->first()->id) }}"
