                                    ...
                                    @foreach ($services->groupBy('inv_no') as $service)
                                        <tr>
                                            ...
                                            <td>
                                                <div class="action">
                                                    <a title="Order Print"
                                                        href="{{ route('pur.order.show', ['item', $client->id, $service->first()->profession_id, $service->first()->customer_card_id, $service->first()->inv_no]) }}"
                                                        class="btn btn-success btn-sm">
                                                        <i class="fas fa-print"></i>
                                                    </a>
                                                    <a title="Invoice Mail"
                                                        href="{{ route('pur.order.viewable_mail', ['item', $client->id, $service->first()->profession_id, $service->first()->customer_card_id, $service->first()->inv_no]) }}"
                                                        class="btn btn-primary btn-sm">
                                                        <i class="far fa-envelope-open"></i>
                                                    </a>
                                                    <a title="Item Edit"
                                                        href="{{ route('service_item.edit', [$client->id, $service->first()->profession_id, $service->first()->customer_card_id, $service->first()->inv_no]) }}"
                                                        class="btn btn-info btn-sm">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <form
                                                        action="{{ route('service_item.destroy', [$client->id, $service->first()->profession_id, $service->first()->customer_card_id, $service->first()->inv_no]) }}"
