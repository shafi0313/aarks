                            ...
                            <table id="examplee" class="table table-striped table-bordered table-hover display table-sm">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($customers as $i => $customer)
                                        @php
                                            $amt = ($customer->dedotrs->sum('amount') + $customer->opening_blnc) - $customer->dedotrPayments->sum('payment_amount');
                                            $total += $amt;
                                        @endphp
                                        @if (($customer->dedotrs->sum('amount') + $customer->opening_blnc) != 0)
                                            <tr>
                                                <td>{{ $i + 1 }}</td>
                                                <td>{{ $customer->name }}</td>
